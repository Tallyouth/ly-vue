    ...
    <div>
      <el-table :data="newTable"
        ...
        <el-table-column label="状态"
          width="200">
          <template slot-scope="scope">
            <span :class="scope.row.statu==1?'el-icon-success':'el-icon-error'"
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            ...
            <el-button size="mini"
              plain
              type="success"
              @click="startPro(scope.$index, scope.row)">
              开始</el-button>
            <el-dialog title="项目介绍"
              ...
              <div slot="footer"
                ...
                <el-button type="primary"
                  @click="dialogFormVisible = false; projectEdit()">确 定</el-button>
              </div>
            </el-dialog>
            <el-button size="mini"
              type="danger"
              @click="stopPro(scope.$index, scope.row)">停止</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
export default {
  ...
  computed: {
    ...
    newTotal () {
      var t = this.tableData.filter((ndata) => {
        return ndata.name.match(this.nname) && ndata.updated_at.match(this.ndate)
      })
      return t.length
    }
